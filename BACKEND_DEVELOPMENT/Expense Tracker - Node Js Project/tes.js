...
window.addEventListener('load', async () => {
    let response = await axios.get('http://localhost:8001/expense/get-expenses')
    try {
...
async function editData(e) {
    ...
    if (e.target.innerHTML == 'Edit') {
        ...
        try {
            ...
            console.log('category :', savedExp.data.category);
...
async function saveData(e) {
    ...
    if (amount.value == '' || description.value == '' || category.value == '') {
        ...
        try {
            ...
            await axios.put(`http://localhost:8001/expense/update-expense/${e.target.id}`, updatedExpense);
            // change save button to edit button after update the expense
